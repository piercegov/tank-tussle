...
void add_past_end(void *l, double i) {
...
void test_resize() {
    ...
    // Fill list - if none of the asserts fail that means the list was resized
    // appropriately

    for (size_t i = 0; i < 10000; i++) {
        add_past_end(l, (double) i);
        assert(vec_equal(*(vector_t*)list_get(l, i),
              (vector_t){ (double) i, (double) i }));
    }
    list_free(l);
}
